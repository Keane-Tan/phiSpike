...
dirN = '../TriggerEff/EcalDCBoundary/withEcalFilter'  # directory where the root files are.
...
sfile = open('Output_Txt/etaPhiRad.txt','r+') # may change
sfile.truncate(0)
...
for fl in flist:
	...
	for var in varList:
		sfile = open('Output_Txt/etaPhiRad.txt','r+') # may change
		sf = sfile.readlines()	
		_file = rt.TFile.Open(dirN+'/'+fl+"/EcalDCBoundaryFilter.root","READ") # may change
		print var+"_"+fl
...
#		Every 2D plots are normalized to the values below
		...
		for xi in range(50):
			for yi in range(50):
				...
				crit = "(Jets[1].Eta() - " + str(etaP) + ")**2 + (Jets[1].Phi() - " + str(phiP) + ")**2 <= rad"

				if var == "AK4jet[0]_etaphi":
					if "Data16" in fl and vrat > 0.25:       # 0.34 for AK4jet[1]_etaphi
						selecV.append(crit)

					if "QCD16" in fl and vrat > 0.5:	
						selecV.append(crit)
